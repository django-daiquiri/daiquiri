...
# Create symlink to old_database
echo "Creating symlink 'old_database' pointing to '$OLD_DB_PATH'..."
ln -sf "$OLD_DB_PATH" old_database
...
docker exec -u postgres pg17 psql -c "CREATE ROLE daiquiri_app SUPERUSER LOGIN;"
docker exec -u postgres pg17 psql -d postgres -c "CREATE DATABASE daiquiri_app OWNER daiquiri_app;"
