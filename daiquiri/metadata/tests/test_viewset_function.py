...
urlnames = {
    'list': 'metadata:function-list',
    'detail': 'metadata:function-detail',
    'management': 'metadata:function-management',
    'user': 'metadata:function-user'
}
...
def test_detail(db, client, username, password, pk):
    ...
    instance = Function.objects.get(pk=pk)
...
def test_create(db, client, username, password):
    ...
    response = client.post(url, {
        'name': f'sleep_{username}_{uuid.uuid4().hex[:6]}',
        'query_string': 'sleep()'
    })
...
def test_delete(db, client, username, password, pk):
    ...
    assert Function.objects.filter(pk=pk).exists() is not (response.status_code == 204)
...
def test_user(db, client, username, password):
    ...
    assert len(response.json()) == (1 if username == 'test' else 0)
